...
function createFirebaseProject(projectId: string): boolean {
  ...
  try {
    execSync(`firebase projects:create ${projectId} --display-name "Join Unified - ${os.userInfo().username}"`,
      { encoding: 'utf8' });
...
function createFirestoreDatabase(projectId: string): boolean {
  ...
  try {
    execSync(`firebase firestore:databases:create "(default)" --project=${projectId} --location=europe-west1`,
      { encoding: 'utf8' });
...
async function main(): Promise<void> {
  ...
  log(`3. Visit Firebase Console: https://console.firebase.google.com/project/${projectId}`, 'cyan');
  log('', 'reset');
